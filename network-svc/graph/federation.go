...
func (ec *executionContext) resolveEntity(
	...
	case "PersonSuggestion":
		resolverName, err := entityResolverNameForPersonSuggestion(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "PersonSuggestion": %w`, err)
		}
		switch resolverName {
		...
		case "findPersonSuggestionByUserID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findPersonSuggestionByUserID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindPersonSuggestionByUserID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "PersonSuggestion": %w`, err)
			}
...
func entityResolverNameForPersonSuggestion(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"user_id\" for PersonSuggestion", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for PersonSuggestion", ErrTypeNotFound))
			break
		}
		return "findPersonSuggestionByUserID", nil
	}
	return "", fmt.Errorf("%w for PersonSuggestion due to %v", ErrTypeNotFound,
