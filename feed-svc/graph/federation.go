...
func (ec *executionContext) resolveEntity(
	...
	case "Post":
		...
		case "findPostByPostID":
			id0, err := ec.unmarshalNID2string(ctx, rep["post_id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findPostByPostID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindPostByPostID(ctx, id0)
			if err != nil {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"post_id\" for Post", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
