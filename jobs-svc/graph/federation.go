...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Job":
		resolverName, err := entityResolverNameForJob(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Job": %w`, err)
		}
		switch resolverName {

		case "findJobByJobID":
			id0, err := ec.unmarshalNID2string(ctx, rep["job_id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findJobByJobID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindJobByJobID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Job": %w`, err)
			}
...
func entityResolverNameForJob(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"job_id\" for Job", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Job", ErrTypeNotFound))
			break
		}
		return "findJobByJobID", nil
	}
	return "", fmt.Errorf("%w for Job due to %v", ErrTypeNotFound,
